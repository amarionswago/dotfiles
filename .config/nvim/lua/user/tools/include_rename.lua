...
local function collect_cpp_files(root, limit)
	local files = {}
	limit = limit or 5000
	...
	local function scan(dir)
		...
		while #files < limit do
			...
			local path = normalize(dir .. "/" .. name)
			if kind == "directory" then
				if not ignored_dirs[name] then
					scan(path)
				end
			elseif kind == "file" and is_cpp_path(path) then
				files[#files + 1] = path
			end
		end
	end
...
local function rewrite_includes_in_project(old_abs, new_abs)
	...
	local files = collect_cpp_files(root, 5000)
...
function M.on_node_renamed(data)
	...
	vim.defer_fn(function()
		local ok, changed_or_err = pcall(rewrite_includes_in_project, old_abs, new_abs)
		if not ok then
			vim.notify("Include rename update failed: " .. tostring(changed_or_err), vim.log.levels.WARN)
			return
		end

		if changed_or_err > 0 then
			vim.notify(("Updated include references in %d file(s)"):format(changed_or_err), vim.log.levels.INFO)
		end
	end, 50)
end
